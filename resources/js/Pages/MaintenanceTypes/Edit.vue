<template>
    <AppLayout title="Type de maintenance-Modification">
            <div class="px-4 sm:px-6 lg:px-8 bg-transparent py-10 h-screen max-w-7xl mx-auto">
                <div class="sm:flex sm:items-center">
                    <Breadcrumbs  class="mb-4"
                                  :pages="[{name:'Type de Maintenance',href:route('maintenancetypes.index'),current:false},{name:'Ajouter',href:route('maintenancetypes.edit',{maintenancetype:item.id}),current:true}]"></Breadcrumbs>
                ...
                <div class="mx-auto mt-5 max-w-5xl flex flex-col gap-4"   @keydown.enter="createOrEditItem()">
                    ...
                    <div class="flex justify-end">
                        <PrimaryButton @click="createOrEditItem()" class="bg-green-600 hover:bg-green-700">Modifier</PrimaryButton>
                    </div>
                </div>
            </div>
        </AppLayout>
...
const props = defineProps({categories:Array,item:Object, errors: Object});
const form = useForm({
   ...props.item
});
const createOrEditItem = ()=>{
    form.put(route('maintenancetypes.update',{maintenancetype:form.id}),form)
}
</script>
