...
const props = defineProps({dailyworker:Array, errors: Object})
const form = useForm(props.dailyworker)
const edit = () => {
    form.put(route('dailyworkers.update',{dailyworker:form.id}), form)
}
</script>

<template>
    <AppLayout title="Ouvrier-Modification">
        <div class="px-4 sm:px-6 lg:px-8 bg-transparent py-10 h-screen max-w-7xl mx-auto">
            <div class="sm:flex sm:items-center">
                <Breadcrumbs class="mb-4"
                             :pages="[{name:'Ouvriers',href:route('dailyworkers.index'),current:false},{name:'Modification',href:route('dailyworkers.edit',{dailyworker:dailyworker.id}),current:true}]"></Breadcrumbs>
