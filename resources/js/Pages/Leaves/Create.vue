...
const props = defineProps({leaveTypes:Array,personnels:Array,errors: Object})
const form = useForm({
    start_date: null,
    end_date: null,
    leavetype_id:null,
...
const add = () => {
    form.post(route('leaves.store'), form)
}
</script>

<template>
    <AppLayout title="Congé-Ajout">
        <div class="px-4 sm:px-6 lg:px-8 bg-transparent py-10 h-screen max-w-7xl mx-auto">
            <div class="sm:flex sm:items-center">
                <Breadcrumbs class="mb-4"
                             :pages="[{name:'Congés',href:route('leaves.index'),current:false},{name:'Ajout',href:route('leaves.create'),current:true}]"></Breadcrumbs>
            ...
            <div class="mx-auto mt-5 max-w-5xl flex flex-col gap-4">
                <div class="flex gap-1 mt-3">
                    ...
                    <FloatLabel class="w-full">
                        <Select class="w-full" v-model="form.leavetype_id" :options="leaveTypes" optionLabel="label"
                                option-value="id" filter/>
                        <label for="car">Type de congé</label>
                    </FloatLabel>
                    <Message v-if="errors.leavetype_id" severity="error" size="small" variant="simple">{{
                            errors.leavetype_id
                        }}
                    </Message>
                    </div>
